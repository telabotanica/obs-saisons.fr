...
const observation = JSON.parse(document.getElementById('observations-data').textContent);
const observationByYearSelected = JSON.parse(document.getElementById('observations-by-year-data').textContent);
...
// Function to check for multiple years and call the appropriate chart functions
    function checkMultipleYears() {
        ...
        if (selectedYears.length > 1) {
            eachYearChart(observationsByYearSelected);
            // combinedSelectedYearChart(observationByYearSelected);
        }
    }

// Get the data from the script tags
    const observations = document.getElementById("observations-data").textContent;
    console.log("test");
    // console.log(observations);
    const observationsByYearSelected = document.getElementById("observations-by-year-data").textContent;
    // console.log(observationsByYearSelected);


// Call the function to create the charts
    checkMultipleYears();

    // Generate the combined year chart with the provided data
